...
async def get_all_connectors_in_repo_dir(
    dagger_client: dagger.Client, repo_dir: dagger.Directory, ignore_glob: Optional[List[str]] = None
) -> Set[Connector]:
    ...
    container_with_repo = dagger_client.container().from_("bash:latest").with_workdir("/repo").with_mounted_directory("/repo", repo_dir)
    all_metadata_files = (await container_with_repo.with_exec(["find", ".", "-name", "metadata.yaml"]).stdout()).splitlines()
    async with asyncer.create_task_group() as task_group:
        tasks = []
        for metadata_file in all_metadata_files:
            if not is_ignored(metadata_file):
                relative_connector_path = str(Path(metadata_file).parent)
            connector_directory = repo_dir.directory(relative_connector_path)
...
async def connectors(ctx: click.Context, pipeline_context: ClickPipelineContext):
    ...
    ctx.obj["all_connectors"] = await get_all_connectors_in_repo_dir(
        dagger_client, ctx.obj["target_repo_state"].repo_dir, ignore_glob=[SCAFFOLD_CONNECTOR_GLOB]
    )
